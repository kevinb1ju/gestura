...
export default function Add() {
    ...
    }, [hoveredIndex, isHoveringSubmit, selectedNotes, currentQuestion, lives, stars, level]);
    ...
    const handleSubmission = (total, target) => {
        if (total === target) {
            const nextStars = stateRef.current.stars + 1;
                ...
                speak(`Game over! You ran out of lives. Returning to dashboard.`, () => {
                    try {
                        ...
                        if (studentId) {
                            ...
                            const baseScore = Math.min(100, stateRef.current.stars * 15 + 30);
