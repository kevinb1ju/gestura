...
const Level2 = ({ onComplete, lives, onWrong }) => {
    ...
    const handleFlowerSelect = (flower) => {
        ...
        if (flower.color === targetColor.value) {
            ...
            setShowFeedback(true);
            const penaltyScore = Math.max(0, score - 5);
            setScore(penaltyScore);
            onWrong(penaltyScore);
    ...
    return (
        <div className="relative w-screen h-screen bg-green-100 overflow-hidden cursor-crosshair font-game">
            ...
            <div className="absolute top-8 left-8 flex gap-4 z-20 items-start">
                <div className="p-6 bg-white/90 rounded-3xl shadow-xl border-b-8 border-green-300 transform hover:scale-105 transition-transform flex items-center gap-6">
                    <div>
                    ...
                    <div className="flex gap-2 bg-white px-4 py-2 rounded-2xl border-4 border-red-200 shadow-inner">
                        {Array.from({ length: 3 }).map((_, i) => (
                            <span key={i} className="text-4xl transition-all duration-300" style={{ opacity: i < lives ? 1 : 0.3, filter: i < lives ? 'none' : 'grayscale(100%)' }}>❤️</span>
                        ))}
                    </div>
                </div>
