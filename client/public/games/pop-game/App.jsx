...
//  Integrated with performance tracking system
// ============================================================
...
function getStudentData() {
  try {
    ...
    if (urlStudentId) {
      console.log('🎮 Pop Game - Student ID from URL:', urlStudentId);
      return urlStudentId;
    }
    ...
    console.log('🎮 Pop Game - Student ID from localStorage:', studentId);
    return studentId;
  } catch (e) {
    console.log('🎮 Pop Game - Using fallback student ID');
    return 'DEMO_STUDENT';
  }
}
...
function initializeGameTracking() {
  ...
  if (window.GameTracker) {
    console.log('🎮 Pop Game tracking initialized for student:', studentId);
    window.GameTracker.setupPopGame(studentId);
  }
}
...
function endGameTracking(finalScore, didWin) {
  if (window.GameTracker) {
    ...
    console.log('🎮 Pop Game ended. Performance data:', result);
...
function GameScreen({ onGameOver }) {
  ...
  const handleMiss = useCallback(id => {
    ...
    showFeedback('� Missed!', '#E91E63');
    speak('Oops!');
    
    // Record missed balloon
    if (window.GameTracker) {
      window.GameTracker.recordIncorrect({ 
        balloon: 'missed',
        reaction_time: Math.random() * 3 + 1, // Slower reaction for miss
  ...
  const handlePop = useCallback((id, px, py) => {
    ...
    // Record successful balloon pop
    if (window.GameTracker) {
      window.GameTracker.recordCorrect({ 
        balloon: 'popped',
        reaction_time: Math.random() * 2 + 0.5, // Simulated reaction time
        position: px + ',' + py
      });
      
      // Record pattern recognition for successful pops
    ...
    // Bonus balloon handling
    const bonus = balloonsRef.current.find(b => b.id === id);
    if (bonus && bonus.emoji) {
      window.SoundManager.bonus();
      speak(bonus.emoji);
      setScore(prev => prev + 2);
      
      // Record bonus achievement
      if (window.GameTracker) {
        window.GameTracker.recordCelebration({ 
          type: 'bonus_balloon',
          emoji: bonus.emoji,
          points: 2
        });
...
function App() {
  ...
  const handleGameOver = (finalScore, didWin) => {
    ...
    // End game tracking
    endGameTracking(finalScore, didWin);
    setScreen('over');
  };
